...
class cgyrodata:
   ...
   def getbigfield(self):
      ...
      # 5. triad
      t,fmt,data = self.extract('.cgyro.triad',cmplx=True)
      
      if fmt != 'null':
         self.triad = np.reshape(data,(self.n_species,self.n_radial,11,self.n_n,nt),'F')
         if not self.silent:
            print('INFO: (getbigfield) Read data in '+fmt+'.cgyro.triad  '+t)
