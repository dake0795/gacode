...
class cgyrodata_plot(data.cgyrodata):
   ...
   def plot_triad_w(self,xin):
      ...
      ax.semilogy(abs(ky[:]),ave_en[:],'*',label=r'$S$',color='k',markersize=4,linestyle='-')
      ax.semilogy(abs(ky[:]),ave_we1[:],'o',label=r'$W_{E,\lambda}$',color='r',markersize=4,linestyle='-')
      ax.semilogy(abs(ky[:]),ave_we2[:],'s',label=r'$W_{E,pol}$',color='b',markersize=4,linestyle='-')
      ax.semilogy(abs(ky[:]),ave_wm[:],'^',label=r'$W_M$',color='g',markersize=4,linestyle='-')
      ax.legend(loc=1,handlelength=1)
      fig.tight_layout(pad=0.4)
      print('S:', ave_en[0], np.sum(ave_en[1:]))
      print('W_E_lam:', ave_we1[0],np.sum(ave_we1[1:]))
      print('W_E_pol:', ave_we2[0],np.sum(ave_we2[1:]))
      print('W_M:', ave_wm[0],np.sum(ave_wm[1:]))
      #======================================
